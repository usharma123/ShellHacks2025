...
# Ensure project root is on sys.path so we can import VC_Analyst.framework
_CURRENT_FILE = Path(__file__).resolve()
_PROJECT_ROOT = _CURRENT_FILE.parents[2]
if str(_PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(_PROJECT_ROOT))
_VC_ANALYST_DIR = _CURRENT_FILE.parents[1]
if str(_VC_ANALYST_DIR) not in sys.path:
    sys.path.insert(0, str(_VC_ANALYST_DIR))

from framework import StartupFramework  # noqa: E402
